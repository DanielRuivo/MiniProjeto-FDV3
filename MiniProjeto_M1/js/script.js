document.addEventListener("DOMContentLoaded", () => {
    ...
    const cadastroAcoesFeedback = document.getElementById("cadastro-acoes-feedback");
    ...
    const relatorioResultadosDiv = document.getElementById("relatorio-resultados");
    ...
    const fakeUsersData = [
        { username: "Ana Reflorestadora", avatar: "pau-brasil", treesPlanted: 1850, bio: "Amante da natureza e plantadora de árvores!" },
        { username: "Beto Castanheira", avatar: "castanheira", treesPlanted: 750, bio: "Contribuindo para um futuro mais verde." },
        { username: "Carla Peroba", avatar: "peroba-rosa", treesPlanted: 310, bio: "Cada árvore conta."},
        { username: "Davi Verde", avatar: "pau-brasil", treesPlanted: 120, bio: "Iniciando minha jornada no reflorestamento."},
        { username: "Elisa Folha", avatar: "castanheira", treesPlanted: 95, bio: ""}
    ...
    function showSection(sectionId) {
        ...
        if (sectionId !== "relatorio" && relatorioResultadosDiv) {
            relatorioResultadosDiv.innerHTML = "<p><i>Faça uma busca para ver os registros.</i></p>";
        }
    ...
    function applyTheme(theme) {
        body.classList.remove("theme-pau-brasil", "theme-castanheira", "theme-peroba-rosa");
        if (theme) {
    ...
    function loadProfileData() {
        ...
        try {
            ...
            const currentUserGlobalData = fakeUsersData.find(u => u.username === userData.username);
            const treeCount = currentUserGlobalData ? currentUserGlobalData.treesPlanted : (userData.treesPlanted || 0);
            ...
            profileTreeType.textContent = treeNameMap[userData.avatar] || userData.avatar;
            ...
            profileAvatarImg.alt = `Avatar ${treeNameMap[userData.avatar]} - ${avatarDetails.stageName}`;
    ...
    function loadHighlightsData() {
        ...
        try {
            ...
            const sortedUsers = [...fakeUsersData].sort((a, b) => b.treesPlanted - a.treesPlanted);

            // Pega os top 3
            const topUsers = sortedUsers.slice(0, 3);

            if (topUsers.length === 0) {
                destaquesListDiv.innerHTML = "<p><i>Nenhum usuário destaque ainda.</i></p>";
            } else {
                let htmlResult = "";
                topUsers.forEach(user => {
                    const avatarDetails = getAvatarDetails(user.avatar, user.treesPlanted);
                    htmlResult += `
            ...
            destaquesListDiv.innerHTML = "<p><i>Ocorreu um erro ao carregar os destaques.</i></p>";
        }
    }
    ...
    function logoutUser() {
        ...
        if (relatorioResultadosDiv) relatorioResultadosDiv.innerHTML = "<p><i>Faça uma busca para ver os registros.</i></p>";
        if (destaquesListDiv) destaquesListDiv.innerHTML = "<p><i>Carregando destaques...</i></p>";
    }
    ...
    if (loggedInUser && savedTheme) {
        ...
        if (savedTheme) {
             applyTheme(savedTheme);
             const savedAvatarOption = document.querySelector(`.avatar-option[data-tree="${savedTheme}"]`);
    ...
    if (cadastroForm) {
        cadastroForm.addEventListener("submit", (event) => {
            ...
            const existingUser = fakeUsersData.find(u => u.username.toLowerCase() === username.toLowerCase());
            if (existingUser) {
    ...
    if (saveBioBtn) {
        saveBioBtn.addEventListener("click", () => {
            ...
            try {
                ...
                const userIndex = fakeUsersData.findIndex(u => u.username === userData.username);
                if (userIndex !== -1) {
    ...
    if (cadastroAcoesForm) {
        cadastroAcoesForm.addEventListener("submit", (event) => {
            ...
            try {
                ...
                setTimeout(() => { cadastroAcoesFeedback.textContent = ""; }, 5000);
    ...
    if (relatorioForm) {
        relatorioForm.addEventListener("submit", (event) => {
            ...
            const userDataString = localStorage.getItem("reflorestamentoUser");
            if (!userDataString) {
                relatorioResultadosDiv.innerHTML = "<p><i>Erro: Usuário não encontrado.</i></p>";
                return;
            }
            ...
            try {
                ...
                if (filteredRecords.length === 0) {
                    relatorioResultadosDiv.innerHTML = "<p><i>Nenhum registro encontrado para os critérios selecionados.</i></p>";
                } else {
                    ...
                    filteredRecords.forEach(record => {
                        ...
                        const speciesName = speciesNameMap[record.species] || record.species;
                ...
                relatorioResultadosDiv.innerHTML = "<p><i>Ocorreu um erro ao gerar o relatório.</i></p>";
            }
        });
    }
